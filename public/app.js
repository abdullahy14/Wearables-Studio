...
const FONTS = ['Inter', 'Helvetica Neue', 'Georgia', 'Arial Black', 'Courier New'];
const SHAPES = [
...
const defaultSurfaces = () => ({
  front: [
    { id: 'headline-front', type: 'text', content: 'Wearables', x: 42, y: 52, fontSize: 34, fontFamily: 'Inter', color: '#0b0b0b' },
    { id: 'subline-front', type: 'text', content: 'Studio Edition', x: 42, y: 114, fontSize: 20, fontFamily: 'Helvetica Neue', color: '#0b0b0b' }
  ],
  back: [{ id: 'headline-back', type: 'text', content: 'Crafted Offline', x: 36, y: 72, fontSize: 28, fontFamily: 'Georgia', color: '#0b0b0b' }],
  leftShoulder: [{ id: 'shoulder-left', type: 'text', content: 'WS', x: 24, y: 24, fontSize: 18, fontFamily: 'Arial Black', color: '#0b0b0b' }],
  rightShoulder: [{ id: 'shoulder-right', type: 'text', content: 'No. 01', x: 24, y: 24, fontSize: 16, fontFamily: 'Courier New', color: '#0b0b0b' }]
});
...
function hasTabletSessionData() {
  return Boolean(state.cart.length || state.placedOrder || state.posGuest.email || state.posGuest.phone || (state.posGuest.name && state.posGuest.name !== 'Walk-in Customer'));
}
function getDraftPayload() {
  return {
    shirtColor: state.designSetup.shirtColor,
    source: state.designSetup.source,
    suggestionId: state.designSetup.suggestionId,
    surfaces: state.designEditor.surfaces,
    drawings: state.designEditor.drawings,
...
function navigate(path) {
  ...
  if (path === '/dashboard' && canAccessAdmin()) ensureAdminExports();
  render();
}
...
function applySuggestion(designId) {
  ...
  if (suggestion) {
    state.designEditor.surfaces.front = [
      { id: 'headline-front', type: 'text', content: suggestion.title, x: 38, y: 48, fontSize: 30, fontFamily: 'Arial Black', color: state.designSetup.shirtColor === 'black' ? '#ffffff' : '#0b0b0b' },
      { id: 'subline-front', type: 'text', content: `Inspired by ${creatorName(suggestion.creatorId)}`, x: 40, y: 104, fontSize: 18, fontFamily: 'Helvetica Neue', color: state.designSetup.shirtColor === 'black' ? '#d6cfc7' : '#4d4d4d' }
    ];
  }
}
...
async function ensureAdminExports() {
  if (!canAccessAdmin()) return;
  const data = await api('/api/design-exports');
  state.adminExports = data.exports;
  render();
}

window.addEventListener('popstate', () => {
  state.currentPath = window.location.pathname;
  if (state.currentPath === '/dashboard' && canAccessAdmin()) ensureAdminExports();
  render();
});
...
document.addEventListener('click', async (event) => {
  ...
  if (type === 'add-text') {
    ...
    surfaceElements().push({ id, type: 'text', content: 'New Text', x: 40, y: 56, fontSize: 24, fontFamily: 'Inter', color: currentTextColor() });
    state.designEditor.selectedIds[activeSurface()] = id;
    render();
  }
  if (type === 'add-shape') {
    const id = `${activeSurface()}-shape-${Date.now()}`;
    surfaceElements().push({ id, type: 'shape', shape: action.dataset.shape, x: 60, y: 80, width: 120, height: action.dataset.shape === 'line' ? 6 : 90, color: state.designSetup.shirtColor === 'black' ? '#d6cfc7' : '#0b0b0b' });
    state.designEditor.selectedIds[activeSurface()] = id;
    render();
  }
  if (type === 'mark-status') {
  ...
  if (type === 'save-design-private') {
    try {
      ...
      await api('/api/design-exports', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ownerId: state.session?.id || 'guest', shirtColor: state.designSetup.shirtColor, sourceDesignId: state.designSetup.suggestionId, surfaces: state.designEditor.surfaces, pngs })
      });
      if (canAccessAdmin()) await ensureAdminExports();
      alert('Design saved privately for admin access.');
    } catch (error) {
      alert(error.message || 'Unable to save design export.');
    }
  }
});
...
document.addEventListener('change', (event) => {
  ...
  if (target.matches('[data-upload="image"]')) {
    ...
    reader.onload = () => {
      ...
      surfaceElements().push({ id, type: 'image', content: String(reader.result), x: 72, y: 148, width: 170, height: 170 });
      state.designEditor.selectedIds[activeSurface()] = id;
      render();
    };
...
document.addEventListener('pointerdown', (event) => {
  ...
  if (state.designEditor.toolMode === 'draw' && canvas.contains(event.target)) {
    ...
    drawingState = { surface: activeSurface(), points: [{ x, y }] };
...
document.addEventListener('pointermove', (event) => {
  ...
  if (!layer) return;
  layer.x = Math.max(0, Math.min(event.clientX - dragState.canvasRect.left - dragState.offsetX, dragState.canvasRect.width - 90));
...
function renderLayer(layer, isSelected) {
  if (layer.type === 'text') {
    return `<div class="layer ${isSelected ? 'selected' : ''}" data-layer-id="${layer.id}" style="left:${layer.x}px;top:${layer.y}px;font-size:${layer.fontSize || 24}px;font-family:${layer.fontFamily || 'Inter'};color:${layer.color || currentTextColor()};font-weight:700;">${escapeHtml(layer.content)}</div>`;
  }
  if (layer.type === 'image') {
    return `<img class="layer ${isSelected ? 'selected' : ''}" data-layer-id="${layer.id}" src="${layer.content}" alt="Uploaded art" style="left:${layer.x}px;top:${layer.y}px;width:${layer.width || 170}px;height:${layer.height || layer.width || 170}px;object-fit:cover;border-radius:16px;" />`;
  }
  if (layer.type === 'shape') {
    const shapeStyle = `left:${layer.x}px;top:${layer.y}px;width:${layer.width || 120}px;height:${layer.height || 90}px;background:${layer.shape === 'line' ? layer.color : layer.shape === 'rect' ? 'transparent' : layer.color};border:${layer.shape === 'rect' ? `3px solid ${layer.color}` : layer.shape === 'circle' ? `3px solid ${layer.color}` : 'none'};border-radius:${layer.shape === 'circle' ? '999px' : layer.shape === 'rect' ? '18px' : '999px'};`;
    return `<div class="layer shape-layer ${isSelected ? 'selected' : ''}" data-layer-id="${layer.id}" style="${shapeStyle}"></div>`;
  }
  return '';
}
function renderDrawingOverlay(surfaceKey = activeSurface(), previewScale = 1, previewClass = '') {
  const drawings = surfaceDrawings(surfaceKey);
  if (!drawings.length) return '';
  return `<svg class="drawing-overlay ${previewClass}" viewBox="0 0 320 360" preserveAspectRatio="none">${drawings.map((stroke) => `<polyline fill="none" stroke="${stroke.color}" stroke-width="${previewScale < 1 ? 2 : 4}" stroke-linecap="round" stroke-linejoin="round" points="${stroke.points.map((point) => `${point.x},${point.y}`).join(' ')}"></polyline>`).join('')}</svg>`;
}
function renderPreviewLayer(layer) {
  if (layer.type === 'text') return `<div class="preview-layer" style="left:${layer.x / 2}px;top:${layer.y / 2}px;font-size:${Math.max(10, (layer.fontSize || 24) * 0.45)}px;font-family:${layer.fontFamily || 'Inter'};color:${layer.color || currentTextColor()};font-weight:700;">${escapeHtml(layer.content)}</div>`;
  if (layer.type === 'image') return `<img class="preview-layer" src="${layer.content}" alt="Preview art" style="left:${layer.x / 2}px;top:${layer.y / 2}px;width:${(layer.width || 170) * 0.45}px;height:${(layer.height || layer.width || 170) * 0.45}px;object-fit:cover;border-radius:10px;" />`;
  if (layer.type === 'shape') return `<div class="preview-layer" style="left:${layer.x / 2}px;top:${layer.y / 2}px;width:${(layer.width || 120) * 0.45}px;height:${(layer.height || 90) * 0.45}px;background:${layer.shape === 'line' ? layer.color : layer.shape === 'rect' ? 'transparent' : layer.color};border:${layer.shape === 'rect' ? `2px solid ${layer.color}` : layer.shape === 'circle' ? `2px solid ${layer.color}` : 'none'};border-radius:${layer.shape === 'circle' ? '999px' : layer.shape === 'rect' ? '10px' : '999px'};"></div>`;
  return '';
}
function renderPreviewSurface(surfaceKey) {
  return `${surfaceElements(surfaceKey).map(renderPreviewLayer).join('')}${renderDrawingOverlay(surfaceKey, 0.45, 'preview-drawing')}`;
}

async function drawLayerToCanvas(ctx, layer) {
  if (layer.type === 'text') {
    ctx.fillStyle = layer.color || currentTextColor();
    ctx.font = `700 ${layer.fontSize || 24}px ${layer.fontFamily || 'Inter'}`;
  ...
  if (layer.type === 'shape') {
    ...
    ctx.lineWidth = 6;
    if (layer.shape === 'rect') ctx.strokeRect(layer.x, layer.y, layer.width || 120, layer.height || 90);
    if (layer.shape === 'circle') { ctx.beginPath(); ctx.ellipse(layer.x + (layer.width || 120) / 2, layer.y + (layer.height || 90) / 2, (layer.width || 120) / 2, (layer.height || 90) / 2, 0, 0, Math.PI * 2); ctx.stroke(); }
    if (layer.shape === 'line') { ctx.fillRect(layer.x, layer.y, layer.width || 120, layer.height || 6); }
    return;
  }
  if (layer.type === 'image') {
    ...
    if (image.complete) ctx.drawImage(image, layer.x, layer.y, layer.width || 170, layer.height || layer.width || 170);
  }
}
async function exportDesignPngs() {
  ...
  for (const surface of SURFACES) {
    ...
    ctx.fillStyle = state.designSetup.shirtColor === 'black' ? '#0b0b0b' : '#ffffff';
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.strokeStyle = state.designSetup.shirtColor === 'black' ? '#1f1f1f' : '#efefef';
    ctx.strokeRect(32, 32, canvas.width - 64, canvas.height - 64);
    ctx.save();
    ctx.scale(3.2, 3);
    for (const layer of surfaceElements(surface.key)) {
      await drawLayerToCanvas(ctx, layer);
    }
    const drawings = surfaceDrawings(surface.key);
    ...
    <div class="shell">
      <header class="header">
        <div class="container header-inner">
          ...
          <nav class="nav">${[['/', 'Home'], ['/design', 'Design Studio'], ['/creator-hub', 'Marketplace'], ['/checkout', 'Checkout'], ['/dashboard', 'Dashboard'], ['/about', 'About']].map(([href, label]) => `<a href="${href}" data-link class="${activeClass(href)}">${label}</a>`).join('')}</nav>
          <div class="actions">
            ${isPosMode() ? '<span class="tag">POS Mode</span>' : ''}
      ...
      <footer class="footer"><div class="container footer-inner"><strong>Wearables Studio</strong><span class="muted">Premium custom apparel · In-store payment only</span></div></footer>
      <nav class="mobile-nav">${[['/', 'Home'], ['/design', 'Studio'], ['/creator-hub', 'Market'], ['/checkout', 'Cart'], ['/dashboard', 'Admin']].map(([href, label]) => `<a href="${href}" data-link class="${activeClass(href)}">${label}</a>`).join('')}</nav>
    </div>
  `;
}
function designCards(designs) {
  return designs.map((design) => `<article class="card hover stack"><img class="product-image" src="${design.imageUrl}" alt="${escapeHtml(design.title)}" loading="lazy" /><div class="row"><div><strong>${escapeHtml(design.title)}</strong><div class="muted">by ${escapeHtml(design.creatorName || creatorName(design.creatorId))}</div></div><span class="badge">${currency(design.price)}</span></div><p class="muted">${escapeHtml(design.description)}</p><div class="row"><span class="eyebrow">${design.salesCount} sales</span><a href="/product/${design.id}" data-link>View Product</a></div></article>`).join('');
}
function qrPanelCard(title, subtitle, payload) {
  return `<div class="qr-card"><div class="eyebrow">${escapeHtml(title)}</div><img class="qr-preview" src="${qrImage(payload)}" alt="${escapeHtml(title)}" /><p class="muted small">${escapeHtml(subtitle)}</p></div>`;
}
  ...
  return shell(`
    <section class="hero card soft">
      <div class="stack">
        ...
        <p class="muted">Wearables Studio combines creator drops, a guided multi-surface builder, and cashier-managed production for a premium store-first experience.</p>
        <div class="toolbar"><a href="/design" data-link class="button">Start Designing</a><a href="/creator-hub" data-link class="button-ghost">Open Marketplace</a></div>
        <div class="grid-3"><div class="card"><div class="eyebrow">Shirt price</div><strong>${currency(brand.price)}</strong></div><div class="card"><div class="eyebrow">Creator commission</div><strong>${currency(brand.commission)}</strong></div><div class="card"><div class="eyebrow">Workflow</div><strong>Pending Payment → Paid → Pickup</strong></div></div>
      </div>
      <div class="card stack"><div class="eyebrow">Store journey</div>${['Choose a shirt color', 'Pick a suggested design or build custom', 'Share or upload via QR', 'Pay with cashier in store', 'Collect with pickup QR'].map((step, index) => `<div class="card row compact"><span>${escapeHtml(step)}</span><span class="tag">0${index + 1}</span></div>`).join('')}</div>
    </section>
    <section class="section"><div><div class="eyebrow">Featured designs</div><h2 class="title-lg">Premium monochrome essentials from the creator marketplace.</h2></div><div class="market-grid">${designCards(designs)}</div></section>
  `);
}
function creatorHubPage() {
  return shell(`<section class="section"><div class="eyebrow">Creator marketplace</div><h1 class="title-lg">A premium gallery where creators publish sale-ready apparel drops.</h1><p class="muted">Customers can browse designs, add them to cart, and complete payment only when they arrive at the store.</p></section>${posPanel('marketplace')}<section class="grid-2">${state.bootstrap.creators.map((creator) => `<div class="card stack"><div class="eyebrow">Creator profile</div><h3>${escapeHtml(creator.name)}</h3><p class="muted">${escapeHtml(creator.bio || 'Premium creator profile.')}</p><div class="row"><span class="badge">EGP ${creator.earnings}</span><a href="/creator/${creator.username}" data-link class="button-ghost">View Profile</a></div></div>`).join('')}</section><section class="market-grid">${designCards(state.bootstrap.designs)}</section>`);
}
...
function productPage(id) {
  ...
  return shell(`<section class="grid-2"><div class="card"><img class="product-image" src="${design.imageUrl}" alt="${escapeHtml(design.title)}" /></div><div class="stack"><span class="badge">Premium custom apparel</span><h1 class="title-lg">${escapeHtml(design.title)}</h1><p class="muted">${escapeHtml(design.description)}</p><div class="card soft stack"><div class="row"><span class="muted">Price</span><strong>${currency(design.price)}</strong></div><div class="row"><span class="muted">Payment model</span><strong>Pay in store only</strong></div><div class="row"><span class="muted">Availability</span><strong>Made to order</strong></div></div><form data-form="add-cart" class="card stack"><input type="hidden" name="designId" value="${design.id}" /><label>Size<select name="size" class="select">${['S', 'M', 'L', 'XL'].map((size) => `<option value="${size}">${size}</option>`).join('')}</select></label><label>Quantity<input name="quantity" class="input" type="number" min="1" max="10" value="1" /></label><div class="toolbar"><button class="button">Add to Cart</button><button type="button" class="button-ghost" data-action="quick-checkout">Checkout</button></div></form></div></section>`);
}

function designerQuestionFlow() {
  if (state.designSetup.step === 1) {
    return `<section class="designer-question card stack"><div class="eyebrow">Step 1</div><h1 class="title-lg">Which base T-shirt would you like to design?</h1><p class="muted">Start by choosing the shirt color before moving into designer suggestions or the custom builder.</p><div class="grid-2"><button class="color-choice black" data-action="set-shirt-color" data-color="black">Black T-shirt</button><button class="color-choice white" data-action="set-shirt-color" data-color="white">White T-shirt</button></div></section>`;
  }
  return `<section class="designer-question stack"><div class="card stack"><div class="eyebrow">Step 2</div><h1 class="title-lg">Start from a top creator suggestion or open the custom designer.</h1><p class="muted">Selected shirt color: <strong>${escapeHtml(state.designSetup.shirtColor)}</strong>.</p></div><div class="market-grid">${topSuggestions().map((design) => `<article class="card hover stack"><img class="product-image" src="${design.imageUrl}" alt="${escapeHtml(design.title)}" /><div><strong>${escapeHtml(design.title)}</strong><div class="muted">by ${escapeHtml(design.creatorName || creatorName(design.creatorId))}</div></div><button class="button" data-action="use-suggestion" data-design-id="${design.id}">Use This Suggestion</button></article>`).join('')}<article class="card stack centered-card"><span class="badge">Custom</span><h3>Open blank designer</h3><p class="muted">Go straight into the editor with a blank multi-surface layout.</p><button class="button-secondary" data-action="use-custom-designer">Open Designer</button></article></div></section>`;
}

function designPage() {
  if (state.designSetup.step < 3) {
    return shell(`${designerQuestionFlow()}`);
  }
  const layer = selectedLayer();
  return shell(`
    <section class="section"><div class="eyebrow">Design studio</div><h1 class="title-lg">Two-sided designer workspace with guided tools on the left and a live preview on the right.</h1><p class="muted">Build across front, back, left shoulder, and right shoulder. Draw freehand, add images, add text in multiple fonts, and wrap artwork with simple shapes.</p></section>
    ${posPanel('design')}
    <section class="designer-board">
      <div class="designer-left stack">
        <div class="card stack">
          <div class="row"><div><div class="eyebrow">Toolkit</div><h3 class="section-title">Editing panel</h3></div><div class="toolbar"><span class="tag">${escapeHtml(state.designSetup.shirtColor)} tee</span><button class="button-ghost" data-action="start-over-designer">Restart</button></div></div>
          <div class="tabs">${SURFACES.map((surface) => `<button class="tab ${activeSurface() === surface.key ? 'active' : ''}" data-action="set-surface" data-surface="${surface.key}">${surface.label}</button>`).join('')}</div>
          <div class="toolkit-grid">
            <button class="button" data-action="add-text">Add Text</button>
            <label class="button-ghost">Add Picture<input data-upload="image" type="file" accept="image/*" hidden /></label>
            ${SHAPES.map((shape) => `<button class="button-ghost" data-action="add-shape" data-shape="${shape.key}">${shape.label}</button>`).join('')}
            <button class="button-ghost ${state.designEditor.toolMode === 'draw' ? 'selected-chip' : ''}" data-action="set-tool" data-tool="draw">Draw</button>
            <button class="button-ghost ${state.designEditor.toolMode === 'select' ? 'selected-chip' : ''}" data-action="set-tool" data-tool="select">Move</button>
          </div>
          <div class="grid-2">
            <label>Text / Label<input class="input" data-input="layer-content" value="${escapeHtml(layer?.content || '')}" ${layer?.type === 'text' ? '' : 'disabled'} /></label>
            <label>Font<select class="select" data-input="layer-font" ${layer?.type === 'text' ? '' : 'disabled'}>${FONTS.map((font) => `<option value="${font}" ${layer?.fontFamily === font ? 'selected' : ''}>${font}</option>`).join('')}</select></label>
          </div>
          <div class="grid-3">
            <label>Font Size<input type="range" data-input="layer-font-size" min="12" max="60" value="${layer?.fontSize || 24}" ${layer?.type === 'text' ? '' : 'disabled'} /></label>
            <label>Color<input class="input color-input" type="color" data-input="layer-color" value="${layer?.color || currentTextColor()}" /></label>
            <label>Draw Color<input class="input color-input" type="color" data-input="stroke-color" value="${state.designEditor.strokeColor}" /></label>
          </div>
          <div class="grid-3">
            <label>X<input type="range" data-input="layer-x" min="0" max="260" value="${layer?.x || 0}" ${layer ? '' : 'disabled'} /></label>
            <label>Y<input type="range" data-input="layer-y" min="0" max="320" value="${layer?.y || 0}" ${layer ? '' : 'disabled'} /></label>
            <label>Width<input type="range" data-input="layer-width" min="40" max="240" value="${layer?.width || 120}" ${(layer && (layer.type === 'image' || layer.type === 'shape')) ? '' : 'disabled'} /></label>
          </div>
          <div class="card soft stack"><div class="eyebrow">Layers on ${SURFACES.find((surface) => surface.key === activeSurface()).label}</div>${surfaceElements().map((item) => `<button class="button-ghost ${state.designEditor.selectedIds[activeSurface()] === item.id ? 'selected-chip' : ''}" data-action="pick-layer" data-id="${item.id}">${escapeHtml(item.type)} · ${escapeHtml(item.id)}</button>`).join('') || '<p class="muted">No layers yet.</p>'}<p class="muted small">Freehand drawing strokes stay on the selected surface and export with the private PNG set.</p></div>
          <div class="canvas-panel ${state.designSetup.shirtColor}" data-design-canvas>
            ${renderDrawingOverlay(activeSurface())}
            ${surfaceElements().map((item) => renderLayer(item, state.designEditor.selectedIds[activeSurface()] === item.id)).join('')}
          </div>
          <div class="toolbar"><button class="button" data-action="save-design-private">Save 4 Private PNGs</button><span class="muted small">Saved exports are visible in the admin dashboard only.</span></div>
        </div>
      </div>
      <div class="designer-right stack">
        <div class="card stack"><div class="row"><div><div class="eyebrow">Live preview</div><h3 class="section-title">Black/white T-shirt render</h3></div><span class="tag">Real-time</span></div><div class="shirt-3d ${state.designSetup.shirtColor}"><div class="shirt-body"></div><div class="shirt-neck"></div><div class="shirt-sleeve left"></div><div class="shirt-sleeve right"></div><div class="preview-surface front">${renderPreviewSurface('front')}</div><div class="preview-surface back">${renderPreviewSurface('back')}</div><div class="preview-surface left-shoulder">${renderPreviewSurface('leftShoulder')}</div><div class="preview-surface right-shoulder">${renderPreviewSurface('rightShoulder')}</div></div><div class="notice">The right side always shows the live multi-surface preview. The saved design export generates four independent PNG files—one for each printable area.</div></div>
      </div>
    </section>
  `);
}
function checkoutPage() {
  return shell(`<section class="section"><div class="eyebrow">Checkout</div><h1 class="title-lg">Create the order online and leave payment to the in-store cashier.</h1><p class="muted">Every order is created as Pending Payment. The cashier updates payment manually when the customer arrives.</p></section>${posPanel('checkout')}<section class="order-grid"><div class="card stack"><h2>Cart summary</h2>${state.cart.length ? state.cart.map((item) => `<div class="card row compact-start"><div class="inline-media"><img src="${item.imageUrl}" alt="${escapeHtml(item.designTitle)}" class="cart-thumb" /><div><strong>${escapeHtml(item.designTitle)}</strong><div class="muted">Size ${item.size} · Qty ${item.quantity}</div></div></div><div class="stack align-end"><strong>${currency(item.quantity * item.price)}</strong><button class="button-ghost" data-action="remove-cart" data-design-id="${item.designId}" data-size="${item.size}">Remove</button></div></div>`).join('') : '<p class="muted">Your cart is empty.</p>'}<div class="row"><span class="muted">Total</span><strong>${currency(totalCart())}</strong></div></div><div class="stack"><form data-form="checkout" class="card stack"><label>Name<input class="input" name="customerName" value="${escapeHtml(isPosMode() ? state.posGuest.name : state.session?.name || '')}" required /></label><label>Phone<input class="input" name="phone" value="${escapeHtml(isPosMode() ? state.posGuest.phone : '')}" placeholder="+20 100 000 0000" required /></label><label>Optional notes<textarea class="textarea" name="notes" rows="4" placeholder="Packaging or pickup notes"></textarea></label><div class="notice">No online payment is collected here. The cashier confirms payment manually in-store before production begins.</div><button class="button" ${state.cart.length ? '' : 'disabled'}>Place Order</button></form>${state.placedOrder ? `<div class="card stack"><div class="eyebrow">Order completed</div><h3 class="section-title">Order ${state.placedOrder.id} created as Pending Payment.</h3><div class="qr-grid">${qrPanelCard('Order QR', 'Use this order QR at the cashier desk.', state.placedOrder)}${qrPanelCard('Pickup QR', 'Use this pickup QR when the order is ready.', { orderId: state.placedOrder.id, purpose: 'pickup', customer: state.placedOrder.customerName })}</div></div>` : ''}</div></section>`);
}
function loginPage() {
...
(async function init() {
  await loadBootstrap();
  if (state.currentPath === '/dashboard' && canAccessAdmin()) await ensureAdminExports();
  render();
})();
